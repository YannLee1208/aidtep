...
if __name__ == '__main__':
    ...
    for dataset_name  in config_data_process.keys():
        ...
        if dataset_config_map.get("use"):
            ...
            data_type = dataset_config_map.get("process.observation.data_type")
            x_sensor_position = dataset_config_map.get("process.observation.x_sensor_position")
            y_sensor_position = dataset_config_map.get("process.observation.y_sensor_position")
            down_sample_factor = dataset_config_map.get("process.observation.down_sample.down_sample_factor")
            down_sample_strategy = dataset_config_map.get("process.observation.down_sample.down_sample_strategy")
            ...
            obs_output_path = obs_output_path.format(data_type=data_type, down_sample_factor=down_sample_factor, down_sample_strategy=down_sample_strategy, x_sensor_position=x_sensor_position, y_sensor_position=y_sensor_position, random_range=random_range, noise_ratio=noise_ratio)
            interpolation_path = interpolation_path.format(method=interp_method, data_type=data_type, down_sample_factor=down_sample_factor, down_sample_strategy=down_sample_strategy, x_sensor_position=x_sensor_position, y_sensor_position=y_sensor_position, random_range=random_range, noise_ratio=noise_ratio)
            ...
            if dataset_config_map.get("process.observation.use"):
                ...
                IA.load_raw_data(dataset_config_map.get("input.obs_path"), data_type)
                if dataset_config_map.get("process.observation.down_sample.use"):
                    logger.info(f"data down sample factor: {down_sample_factor}, strategy: {down_sample_strategy}")
                    IA.down_sample_raw_data(down_sample_factor, down_sample_strategy)
                ...
                IA.get_observation(x_sensor_position, y_sensor_position, random_range, noise_ratio)
